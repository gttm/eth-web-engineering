...
$(document).ready(function() {
    ...
    if (animated["welcome"] == false) {
        if ($('#welcome-container').isOnScreen() == true) {
            $('#welcome-container').animate({marginTop: '0', marginBottom: '0'}, 1000);
            animated["welcome"] = true;
        }
    }
    ...
    $(window).scroll(function() {
        ...
        if (animated["cuisine"] == false) {
            if ($('#cuisine-container').isOnScreen() == true) {
                $('#cuisine-container-text').animate({marginLeft: '0', marginRight: '0'}, 1000);
                animated["cuisine"] = true;
            }
        }

        if (animated["ingredients"] == false) {
            if ($('#ingredients-container').isOnScreen() == true) {
                $('#ingredients-container').animate({marginLeft: '0', marginRight: '0'}, 1000);
                animated["ingredients"] = true;
            }
        }

        if (animated["events-header"] == false) {
            if ($('#events-header').isOnScreen() == true) {
                $('#events-header').animate({marginLeft: '0', marginRight: '0'}, 1000);
                animated["events-header"] = true;
            }
        }
